...
public class YumController {
	...
	public void run() {
		...
		while (true) {
			System.out.println("1. 등록 | 2. 전체조회 | 3. 개별조회 | 4. 수정 | 5. 삭제 | 6. 통계 | 0. 종료");
	...
	private void addYum() {
		...
		/*
		 * if(addResult) { System.out.println("등록 성공"); } else {
	...
	private void outReport() {
		...
		System.out.println("전체 맛집 수: " + totalCnt + "개");
		
		// 평균 평점 (소수점 자리)
		double avg = service.getAvgRating();
		System.out.printf("전체 평균 평점: %.1f점\n", avg); // + 아니고 , 로 넣어야 함
		
		// 4점 이상 맛집 수 
		int cnt4 = service.getCountRating(4);
		System.out.println("4점 이상 맛집 수: "+ cnt4 + "개");
